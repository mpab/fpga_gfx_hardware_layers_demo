...
module demo_sinescroll #(
  ...
  logic lb_line;
  always_ff @(posedge clk_sys) begin
    if (line0_sys) lb_line <= 1;  // enable from beam_y==0
  ...
  always_ff @(posedge clk_pix) begin
    lb_en_out <= (beam_y >= FB_OFFY && beam_y < (FB_HEIGHT * FB_SCALE) + FB_OFFY
            && beam_x >= FB_OFFX - LAT_LB && beam_x < (FB_WIDTH * FB_SCALE) + FB_OFFX - LAT_LB);
  end
  ...
  always_comb begin
    paint_area = (beam_y >= FB_OFFY && beam_y < (FB_HEIGHT * FB_SCALE) + FB_OFFY
            && beam_x >= FB_OFFX && beam_x < FB_WIDTH * FB_SCALE + FB_OFFX);
